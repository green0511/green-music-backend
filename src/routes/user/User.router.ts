...
class UserRouter {
  ...
  constructor() {
    ...
    this.router.get('/:userId', this.getUserInfo)
  ...
  public async getUserAllLists(req: Request, res: Response) {
    ...
    let findList = await List.find(query, page, length)
    let count = await List.getCount(query)
  ...
  public async createListForUser(req: Request, res: Response) {
    ...
    let findList = await List.findByName(name, user)
    if (findList) {
      return res.json({success: false, msg: '歌单名重复'})
    }

    let list = await List.create(<IList>{name, desc, cover}, user)
